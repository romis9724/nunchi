...
- **막힌 곳**: pgvector 미사용, 캘린더 DB 미연동, 회귀 테스트 부재, 사용자 인터뷰 미시작
- **다음 1주**: 데이터 19건 추가 + 회귀 테스트셋 + 캘린더 DB 연동
...
### 데이터 (`data/events/`)
- [x] **31개 JSON 큐레이션** — 모두 의미 있는 summary, 정부/학술 출처 링크, related_keywords·motifs 명시
- [x] 큐레이션된 핵심 사건:
  - 정치 트라우마: 5·18 광주, 4·19 혁명, 10·26 김재규, 12·12 쿠데타, 6·10 항쟁, 노무현 서거
  - 재난: 세월호 4·16, 이태원 10·29, 천안함 3·26, 연평도 11·23, 용산참사 9·29
  - 학살·인권: 4·3 제주, 위안부 1·28
  - 국가기념일: 광복절 8·15, 한글날 10·9, 현충일 6·6, 개천절 10·3, 3·1절, 제헌절 7·17, 어린이날·어버이날·스승의 날
  - 일반 기념일: 발렌타인·화이트데이·어린이날·할로윈·빼빼로데이·크리스마스·신정·새해
...
- 같은 문서의 `reviews` 테이블에 `grade` 컬럼이 없지만, 실제 코드는 `grade`를 upsert함 — 스키마와 마이그레이션 추가 필요.
...
- [ ] 사건 19건 추가 큐레이션 (31 → 50)
  - 후보: 부마항쟁(10·16), 제2연평해전(6·29), 광화문 촛불(겨울 시즌), 4·11 총선/대선 시즌, 노태우 서거(10·26 같은날 충돌), 김대중 서거(8·18), 일본 종전일(8·15와 충돌), 한일 위안부 합의일, 6자회담 등
- [ ] `seed.ts`에 임베딩 생성 추가
- [ ] `review-engine.ts`에서 pgvector 유사도 검색 활성화

### Phase 2 — 검증
- [ ] 회귀 테스트셋: 5·18+탱크, 4·16+리브랜드, 8·15+독립(A등급), 평일+신상(B등급), 1·29+코미디 등
- [ ] CI에서 `pnpm test:regression` 실행 (root package.json에 스크립트 자리 있음, 구현 미)
