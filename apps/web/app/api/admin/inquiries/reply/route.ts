...
export async function POST(request: NextRequest) {
  ...
  try {
    await sendMail({
      to: body.to,
      subject: `[nunch-i] 문의 답장 — ${body.name ?? ""}`,
      html: `
        <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px">
          <div style="margin-bottom:24px">
            <h2 style="color:#0078D4;margin:0 0 4px">nunch-i 팀입니다</h2>
          ...
          <p style="font-size:12px;color:#999;margin:0">nunch-i — 캠페인 날짜 리스크 분석 서비스</p>
        </div>
      `,
    });
