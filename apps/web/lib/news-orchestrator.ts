 ...
 *
 * 삽입은 events 리포지토리(RDS)를 통한다.
 */

import type { NewsAutomationResult } from "./news-automation.js";
import { insertNewsDraftEvent } from "./repositories/events.repo";
...
export async function runNewsOrchestrator(): Promise<NewsAutomationResult> {
  ...
  // 3. Insert drafts into events table (RDS)
  for (const item of filtered.slice(0, 10)) {
    ...
    try {
      ...
      const inserted = await insertNewsDraftEvent({
        slug,
        month: d.getMonth() + 1,
        day: d.getDate(),
        name: title,
